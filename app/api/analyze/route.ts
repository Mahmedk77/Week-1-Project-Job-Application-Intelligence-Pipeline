...
export async function POST( request: Request) {
    ...
    try {
        ...
        const jobParserRes = await jobParserChain.invoke({ jobDesp: jobDescription });
        ...
        const resumeParserRes = await resumeParserChain.invoke({ resume: resumeText });
        ...
        return new Response( readable, {
            headers: {
                ...
                "X-Job-Parser": Buffer.from(JSON.stringify(jobParserRes)).toString("base64"),
                "X-Resume-Parser": Buffer.from(JSON.stringify(resumeParserRes)).toString("base64"),
                "X-Match-Score": Buffer.from(JSON.stringify(matchScore)).toString("base64"),
            }
        })
